...
def create_branch_and_commit(repo_full_name: str, branch_name: str, file_path: str, file_content: str, commit_message: str):
    ...
    try:
        ...
        # Delete stale branch so the PR diff is always clean
        existing_ref.delete()
...
def create_pr_node(state: AgenticState) -> dict:
    ...
    try:
        ...
        # Try to load the RCA JSON and render it nicely in the PR body
        rca_path = AGENTIC_TMP_DIR / f"iter_{iteration}_rca.json"
        rca_text = ""
        if rca_path.exists():
            try:
                rca_obj = json.loads(rca_path.read_text())
                rca_text = (
                    f"**Summary:** {rca_obj.get('summary', 'N/A')}\n\n"
                    f"**Root Cause:** {rca_obj.get('root_cause', 'N/A')}\n\n"
                    f"**Patch Summary:** {rca_obj.get('patch_summary', 'N/A')}"
                )
            except Exception:
                rca_text = rca_path.read_text()
        if not rca_text:
            rca_text = "RCA could not be generated for this iteration."
        pr_title = f"agentic: auto-fix (iter {iteration})"
        pr_body = (
            f"## Automated fix from Pipeline Doctor (iteration {iteration})\n\n"
            f"### Root Cause Analysis\n\n{rca_text}\n\n"
            f"---\n_Generated by the Self-Healing Agentic CI/CD pipeline._"
        )
        log('INFO', "-> Opening PR...")
